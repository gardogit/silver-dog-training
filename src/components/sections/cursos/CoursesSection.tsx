...
export const CoursesSection: React.FC = () => {
    ...
    const [selectedService, setSelectedService] = useState<Package | { title: string } | null>(null);

    const handleRequestService = (service: Package | { title: string }) => {
    ...
    const renderContent = () => {
            ...
            return (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {currentData.packages.map((pkg) => (
                        <PackageCard key={pkg.title} pkg={pkg} onRequest={handleRequestService} />
                    ))}
                </div>
            );
        }
        ...
        if ('content' in currentData) {
            return <K9ServicePanel content={currentData.content} onRequest={handleRequestService} />;
        }
